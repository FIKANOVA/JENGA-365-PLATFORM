-- ─── 0013: v_public_impact_aggregate ──────────────────────────────────────
--
-- CLAUDE.md §11: public-facing Looker Studio dashboard reads unfiltered
-- aggregate totals from this view (no partner-id filter). Drives the
-- "X trees planted, Y youth engaged" surface on (marketing)/impact and /about.
--
-- Tree counting uses the planting events table (CLAUDE.md §10.4). Tree
-- survival uses DISTINCT ON (project_location_id) ORDER BY survey_date DESC
-- per CLAUDE.md §2 to avoid double-counting audits.
