...
export async function GET(req: Request) {
  ...
  try {
    ...
    const donationsRes = await db.select({
    ...
    const donationStats = donationsRes[0];
    const sessionStats = sessionsRes[0];

    await db.insert(impactReports).values({
      reportPeriod: new Date().toLocaleString("default", {
        month: "long",
        year: "numeric",
      }),
    ...
    return NextResponse.json({
      success: true,
      message: "Nightly impact report generated.",
    });
  } catch (err: any) {
    return NextResponse.json(
