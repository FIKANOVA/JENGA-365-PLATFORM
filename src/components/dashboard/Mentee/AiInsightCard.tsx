...
export default function AiInsightCard({ profileMatch, deepSkillMatch, className = "" }: AiInsightCardProps) {
    return (
        <div className={`mt-4 p-4 rounded-md bg-primary/5 border border-primary/10 space-y-3 animate-in fade-in slide-in-from-top-2 duration-500 ${className}`}>
            ...
            <p className="text-[11px] leading-relaxed text-muted-foreground bg-white/50 dark:bg-black/20 p-2 rounded-md border border-black/5 dark:border-white/5 italic">
                {deepSkillMatch > 80
